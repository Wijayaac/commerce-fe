        ...
        <ul class="header-links d-flex">
          ...
          <li>
            <NuxtLink class="header-link" to="/cart">
              Cart : <span class="counter">{{ cart.total }}</span>
            </NuxtLink>
          </li>
          <li>
            <div class="user">
...
<script setup>
import { useCartCounter } from "../../stores/cart";
...
// const { data: carts } = await client.from("carts").select("id");
// cartCounter.value = carts.length;

// const counterState = useState("counter", () => cartCounter.value);
cart.syncData();
</script>
